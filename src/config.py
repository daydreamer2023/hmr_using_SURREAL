...
curr_path = '/home/cha/SamSung/hmr'
model_dir = '/home/cha/SamSung/hmr/models'
...
PRETRAINED_MODEL = osp.join(model_dir, 'model.ckpt-51608')
CONTINUED_MODEL = osp.join(curr_path, 'src/logs/HMR_3DSUP_mpi_inf_3dhp_CMU-H3.6-jointLim_resnet_fc3_dropout_Elr1e-03_kp-weight60_Dlr1e-03_Nov28_1235')
...
flags.DEFINE_string('pretrained_model_path', None,
                    'if not None, fine-tunes from theis ckpt')
...
DATA_DIR = '/home/cha/SamSung/DataSet/tf_datasets/'

flags.DEFINE_string('data_dir', DATA_DIR, 'TFRecord dataset location')
...
flags.DEFINE_list('datasets', ['SURREAL_Basic'], 'datasets to use for training')
...
def prepare_dirs(config, prefix=['HMR']):
    ...
    # if there is any pretrained config
    if config.load_path:
